"""
weforge.registry.schema
=======================
SQLite schema for the W.E. FORGE production registry.
...
REGISTRY_DIR = Path.home() / ".weforge" / "registry"
REGISTRY_PATH = REGISTRY_DIR / "weforge.db"
SCHEMA_VERSION = 1
...
def initialize_registry(db_path: Path = REGISTRY_PATH) -> sqlite3.Connection:
    ...
    cursor.execute(
        ...
        (SCHEMA_VERSION, "Initial schema — W.E. FORGE v1.0")
    )
    conn.commit()
    return conn
