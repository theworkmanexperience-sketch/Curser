# W.E. FLOW — Phase 0 Operator Guide
...
- The `we_flow/` codebase from this package
...
Navigate to the `we_flow/` directory and install the one required dependency:
...
The very first time you run the engine interactively, it will display the W.E. FLOW End User License Agreement (EULA v1.0). This is a one-time requirement.
...
> **Non-interactive / CI environments:** If `stdin` is not a terminal (e.g., you are running from a script), the EULA prompt is skipped and the version is recorded automatically. Use `WEFLOW_NONINTERACTIVE=1` to force this mode.
...
For questions about W.E. FLOW or this guide, contact:
...
*W.E. FLOW Phase 0 Operator Guide | v4.6 | May 22, 2026*  
