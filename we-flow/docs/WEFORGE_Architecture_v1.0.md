# W.E. FORGE — Technical Architecture Document
...
> *"W.E. FORGE at J5 is a production intelligence platform with 2+ years of creator data  
...
W.E. FORGE is a Production Intelligence Platform for independent video creators and small production companies. It combines automated media ingest, multi-camera temporal alignment, AI-assisted content analysis, and a persistent production registry into a single local-first platform.

W.E. FLOW is the first product from W.E. FORGE — an automated ingest, scaffold, and proxy generation tool that replaces four separate workflow tools with a single command. W.E. FLOW is the proof of concept that earns the platform narrative.

The platform is designed along a five-juncture AI intelligence roadmap. At Juncture 1, W.E. FORGE is a faster, smarter ingest tool. At Juncture 5, it is the only platform that owns production intelligence for the independent creator — a persistent AI trained on a creator's entire production history, operating locally, with no footage ever leaving the creator's machine without explicit consent.

**The market gap W.E. FORGE fills:**  
Adobe owns the cloud. Apple owns the edit. Blackmagic owns color. Nobody owns pre-edit ingest intelligence for independent creators at scale. W.E. FORGE owns that gap.
...
Every creator using W.E. FORGE operates under three guarantees:
...
|----------|------------|-----------|
| **Brand leads** | W.E. FORGE is announced before W.E. FLOW ships | Platform narrative must precede product launch |
...
| **In-house core** | W.E. FLOW, W.E. EDIT, W.E. ARCHIVE built internally | Vision and quality bar cannot be licensed |
...
### P1 — Determinism
Given identical input and configuration, W.E. FORGE produces identical output. Every stage is reproducible, testable, and verifiable. No randomness, no network calls, no environmental dependencies in the core pipeline.
...
## 4. W.E. FORGE Platform Structure

```
W.E. FORGE — The Production Intelligence Platform
│
├── PRODUCTS
│   ├── W.E. FLOW        ← Ingest, scaffold, proxy (shipping)
...
## 5. W.E. FLOW — Product Specification

### 5.1 Definition

W.E. FLOW is an automated multi-camera ingest, scaffold, and proxy generation tool. It ingests raw footage from any combination of cameras, corrects corrupted metadata, organizes footage into a structured scaffold, generates editorial proxies, and produces a complete audit manifest — in a single automated run.
...
Editorial intelligence layer. Receives scaffold and proxy output from W.E. FLOW and produces:
...
**Technical dependency:** W.E. EDIT is a collection of pipeline stages operating on the same registry as W.E. FLOW. No separate data layer required.
...
**Technical dependency:** W.E. ARCHIVE queries the production registry built by W.E. FLOW from day one. Users who have run W.E. FLOW for 18 months have 18 months of archive intelligence on day one of W.E. ARCHIVE.

### 6.3 W.E. API (J3 Roadmap)

Public extension layer exposing the W.E. FORGE pipeline interface to third-party developers:
...
The package namespace is reorganized from `we_flow/` to `weforge/` before v1 ships. This is the last point at which renaming is cost-free.

```
weforge/
│
├── core/                    ← Shared across all W.E. FORGE products
│   ├── __init__.py
...
1. `weforge/core/` has zero imports from `weforge/flow/`, `weforge/intelligence/`, or `weforge/sync/cloud/`
2. `weforge/flow/` imports from `weforge/core/` and `weforge/registry/` only
3. `weforge/intelligence/` imports from `weforge/core/` and `weforge/registry/` only
4. `weforge/sync/cloud/` is never imported by `weforge/flow/` directly — only through `weforge/core/sync.py` interface
5. `weforge/api/` imports from `weforge/core/` only — the API is a thin contract layer

---

## 8. Pipeline Stage Interface

Every processing stage in W.E. FORGE — current and future — implements this interface. This is the extension point that makes the plugin API possible.

```python
# weforge/core/stage.py
...
```sql
-- weforge/registry/schema.py

-- RUNS: One record per W.E. FORGE pipeline execution
CREATE TABLE IF NOT EXISTS runs (
    id                  TEXT PRIMARY KEY,    -- UUID v4
    timestamp           TEXT NOT NULL,       -- ISO 8601 UTC
    we_forge_version    TEXT NOT NULL,       -- Platform version
    we_flow_version     TEXT,                -- W.E. FLOW version if applicable
...
class RunManifest:
    ...
    def to_fcpxml(self) -> str:
        ...
        """
        return '<?xml version="1.0" encoding="UTF-8"?><!-- W.E. FORGE FCPXML stub -->'
    ...
    def to_drxml(self) -> str:
        ...
        v1: Empty stub — interface exists, implementation at J4.
        """
        return '<!-- W.E. FORGE DaVinci XML stub -->'
```
...
```python
# weforge/api/extensions.py

from typing import Type, Dict
from weforge.core.stage import PipelineStage
...
class ExtensionRegistry:
    ...
    W.E. FORGE products (W.E. EDIT, W.E. ARCHIVE) use the same registry.
    """
...
from weforge.flow.ingest import IngestStage
from weforge.flow.archive import ArchiveIntelligenceStage
from weforge.flow.scaffold import ScaffoldStage
from weforge.flow.proxy import ProxyStage
from weforge.flow.audit import AuditStage
...
**W.E. FORGE platform value:** Platform accumulates camera fingerprint library across creator community. Rare and new cameras become identifiable faster than any manual approach.
...
**Pricing impact:** Justifies W.E. FLOW upgrade from $99 → $149/year.

**W.E. FORGE platform value:** Quality scoring model improves with each creator's feedback over time. The platform learns what "good footage" means per creator, per shoot type, per client.
...
**Pricing impact:** Justifies W.E. FLOW upgrade to $199/year. Eliminates $199 competitor purchase.

**W.E. FORGE platform value:** Public API opens at J3. Third-party developers can build custom alignment algorithms, specialized classifier plugins, and NLE-specific export formats. Ecosystem begins.

**Competitive impact:** PluralEyes (Red Giant/Maxon) charges $199 for this single feature as a standalone tool. W.E. FORGE includes it in the standard ingest run. No other sub-$500 tool offers this capability.
...
**W.E. FORGE platform value:** W.E. EDIT launches as a distinct product at this juncture — with W.E. FLOW as the ingest layer and W.E. EDIT as the editorial intelligence layer. Platform narrative fully earned.

**Pricing impact:** Justifies $249-299/year positioning. The tool now touches the editorial phase — not just ingest.

**Acquisition signal:** This is the feature set that attracts acquisition conversations. Adobe, Apple, and Blackmagic are all building editorial AI. W.E. FORGE has it running locally, privately, at a prosumer price point.
...
- Cloud sync: Optional W.E. FORGE cloud sync for cross-device continuity
...
**W.E. FORGE platform value:** At J5, W.E. FORGE is no longer a tool. It is a production intelligence platform trained on 2+ years of a creator's work. A new user starting at J5 starts from zero. An existing user starting at v1 has 2 years of production intelligence on day one of J5.
...
Gate: W.E. FLOW processes 100-file shoot in under 90 minutes

Build items:
- proxy_workers: 4 (parallel transcoding)      Est. 2 weeks
- Pre-flight ffprobe duration estimate          Est. 1 week
- Package namespace reorganization (weforge/)   Est. 1-2 days
...
Deliverable: W.E. FORGE brand + W.E. FLOW v1 public release

Build items:
- W.E. FORGE brand identity + weforge.com          Est. 1-2 weeks
...
- Optional cloud sync (W.E. FORGE Cloud)          Est. 8-12 weeks
...
| Capability | Hedge | ShotPut Pro | Kyno | Silverstack | PluralEyes | W.E. FORGE v1 | W.E. FORGE J5 |
|------------|-------|-------------|------|-------------|------------|----------------|----------------|
| Verified copy | ✅ Best | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ |
...
**W.E. FORGE workflow (one platform, one run):**

```
Card → W.E. FORGE ($149-299/yr) → Verified + organized + aligned + proxied
                                   Total cost: $149-299/yr
                                   Total time: <1 hour (J3 complete)
```

**At J3, W.E. FORGE replaces $448/year of tools with a single $199/year platform at half the cost and half the time.**
...
## 22. W.E. FORGE at J5 — The Differentiated Position

W.E. FORGE at full J5 maturity:
...
**The acquisition conversation for W.E. FORGE is not about users or ARR alone — it is about production intelligence infrastructure that cannot be rebuilt from scratch.**

Any acquirer that buys W.E. FORGE also acquires:
...
|-----------|---------------|---------------|
| Phase 3 launch | W.E. FORGE v1, 500-1,000 users | Proven PMF, recurring revenue |
...
1. **Prove the core before adding intelligence.** W.E. FLOW v1 ships with zero AI — the workflow proves market fit. AI features prove scale.
...
3. **Privacy is the moat, not a constraint.** Local-first architecture is technically verifiable. "Your footage never leaves your machine" is an engineering guarantee, not a marketing claim. This positions W.E. FORGE against Adobe in a way no cloud-first competitor can match.
...
5. **Platform pricing is earned, not assumed.** W.E. FORGE launches at $149/year. The price increases as intelligence features ship and are validated by real users. Pricing is locked to demonstrable value.

---

*Document version 1.0 — Architecture Lock*  
*Next review: Phase 2 completion (proxy_workers:4 benchmarked)*  
*Distribution: W.E. FORGE core team + prjctlazrus.com (under executed NDA)*
...
*W.E. FLOW: The first product from W.E. FORGE.*
